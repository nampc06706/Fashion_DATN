...
export default function PasswordTab() {
  ...
  let username = null;
  let email = ""; // Thêm biến để lưu email

  if (token) {
    try {
      const decodedToken = jwtDecode(token); // Giải mã token
      username = decodedToken.sub || "";
      email = decodedToken.email || ""; // Lấy email từ decodedToken
    } catch (error) {
  ...
  if (!oldPassword || !newPassword || !confirmPassword) {
    ...
    const payload = {
      email: email, // Sử dụng email từ decodedToken
    ...
    try {
      const response = await axios.post("http://localhost:8080/api/update-password-profile", payload);
      ...
      if (response.status === 200) {
        ...
        toast.success("Cập nhật mật khẩu thành công!"); // Hiện thông báo thành công
        // Có thể làm thêm gì đó như redirect hoặc reset form
      } else {
        toast.error(`Lỗi: ${response.data.message || "Không thể cập nhật mật khẩu."}`);
      }
    } catch (error) {
      console.error("Có lỗi xảy ra:", error);
      toast.error("Mật khẩu cũ không khớp"); // Hiện thông báo lỗi
    }
  };
