...
function ProductCardStyleThree({ datas }) {
  ...
  // Làm tròn và định dạng giá trị tiền tệ
  const formattedPrice = datas.price ? `${Math.round(datas.price).toLocaleString("vi-VN")} đ` : "N/A";
  const formattedDiscount = datas.discount ? `${Math.round(datas.discount).toLocaleString("vi-VN")} %` : "N/A";
  const formattedOriginalPrice = datas.originalPrice ? `${Math.round(datas.originalPrice).toLocaleString("vi-VN")} đ ` : "N/A";
  ...
  return (
    <div className="product-cart-three w-full group">
      <div className="w-full h-[364px] bg-white flex justify-center items-center p-2.5 mb-6 relative overflow-hidden">
        <img src={`/assets/images/${datas.firstImage}`} alt={datas.name} />
        <div className="quick-access-btns flex flex-col space-y-2 absolute right-4 top-5 transition-all duration-300 ease-in-out">
          <a href="#" className="flex justify-center items-center">
            <span className="w-12 h-12 flex justify-center items-center bg-red-600 rounded-full shadow-lg transform hover:scale-105 transition-transform duration-300 relative">
              <span className="text-white font-bold text-lg absolute">
                {formattedDiscount}
              </span>
            </span>
          </a>
          <span className="text-xs font-bold text-red-600 mt-1">Giảm giá</span> {/* Chú thích cho phần giảm giá */}
        </div>
        <div className="absolute w-full h-10 px-[30px] left-0 -bottom-10 group-hover:bottom-5 transition-all duration-300 ease-in-out">
          <button
            type="button"
            className="black-btn w-full h-full flex justify-center items-center bg-qblack text-white rounded-lg"
            onClick={handleDetailClick} // Gọi hàm điều hướng khi nhấp vào nút
          >
      ...
      <div className="flex flex-col items-center">
        <h2 className="text-xl font-semibold text-qblack mb-2 text-center">
          {datas.name}
        </h2>
        {/* Giá gốc */}
        <p className="text-base font-medium text-qgraytwo text-center line-through">
          {formattedOriginalPrice} {/* Giá gốc */}
        </p>
        {/* Giá mới */}
        <p className="text-lg font-bold text-red-600 text-center">
          {formattedPrice} {/* Giá đã giảm */}
        </p>
      </div>
